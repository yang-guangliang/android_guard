...
class EncodedAnnotation(object):
    ...
    def __init__(self, buff, cm, visibility = 1):
        self.CM = cm
        self.offset = buff.get_idx()

        self.visibility = visibility
        self.visibility_value = visibility_dict.get(self.visibility, "runtime")
...
class AnnotationItem(object):
    ...
    def __init__(self, buff, cm):
        ...
        self.annotation = EncodedAnnotation(buff, cm, self.visibility)
...
class ClassDataItem(object):
    ...
    def get_methods(self):
        """
            ...
            :rtype: a list of :class:`EncodedMethod` objects
        """
        result = [x for x in self.direct_methods] + [x for x in self.virtual_methods]
        return set(result)
    ...
    def get_fields(self):
        """
            ...
            :rtype: a list of :class:`EncodedField` objects
        """
        result = [x for x in self.static_fields] + [x for x in self.instance_fields]
        return set(result)
...
class DvmAnnotations(object):
        ...
        def build(self):
                ...
                self.annotations_set_ref_list = self.map_list.get_item_type("TYPE_ANNOTATION_SET_REF_LIST")

        """
        result = {
                key (method_info [clazz_name, method_name, descriptor])  :  [content (EncodedAnnotation)],
        }
        ...
        def get_all_methods_with_annotations(self):
                ...
                result = {}
                for method_annotation in all_methods_annotations:
                        method_info = self.get_cm_method(method_annotation.get_method_idx())
                        if method_info[0].startswith("Landroid/support"):
                                continue

                        method_info_string = "%s%s%s" % (method_info[0], method_info[1], ''.join(method_info[2]))
                        result[method_info_string] = set()

                        # get annotation_set_item from method_annotation
                        self.set_idx(method_annotation.get_annotations_off())
                        annotation_set_item = AnnotationSetItem(self, self.CM)

                        for off_item in annotation_set_item.annotation_off_item:
                                # get annotation_item
                                self.set_idx(off_item.annotation_off)
                                annotation_item = AnnotationItem(self, self.CM)

                                # get encoded_annotation
                                encoded_annotation = annotation_item.annotation
                                result[method_info_string].add(encoded_annotation)
                return result
        ...
        All return values are saved into the set `result`.
        ...
        """
        def parse_EncodedAnnotation(self, result, annotation):
                for elem in annotation.elements:
                        if isinstance(elem.value.value, EncodedAnnotation):
                                self.parse_EncodedAnnotation(result, elem.value.value)
                        elif isinstance(elem.value.value, EncodedArray):
                                self.parse_EncodedArray(result, elem.value.value)
                        else:
                                result.add(elem.value.value)

        def parse_EncodedArray(self, result, array):
                for value in array.values:
                        if isinstance(value.value, EncodedAnnotation):
                                self.parse_EncodedAnnotation(result, value.value)
                        elif isinstance(value.value, EncodedArray):
                                self.parse_EncodedArray(result, value.value)
                        else:
                                result.add(value.value)



class DalvikVMFormat(bytecode._Bytecode, DvmAnnotations):
    """
    ...
    def get_classes(self):
        ...
        """
        return set(self.classes.class_def)
